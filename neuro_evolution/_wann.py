...
from ._wann_reproduce import WannReproduction
...
class BaseNEAT(BaseEstimator, metaclass=ABCMeta):
    ...
    @abstractmethod
    def __init__(self, single_shared_weights, number_of_generations, fitness_criterion, fitness_threshold, pop_size,
    ...
    def _set_neat_config(self, num_inputs, num_outputs):
        ...
        _genome_config = GenomeConfig(self.activation_default, self.activation_mutate_rate, self.activation_options,
                                      ...
                                      self.num_hidden, num_inputs, num_outputs, self.response_init_mean,
                                      self.response_init_stdev, self.response_max_value, self.response_min_value,
        ...
        _config = ConfigParser(WannGenome, WannReproduction, neat.DefaultSpeciesSet,
    ...
    def _fit(self, X, y):
        """
        Validate the inputs.
        Assign private NEAT config variables.
        Run the WANN algorithm.
        Displays the best genome.
        ...
        Returns
        -------
        self : WANNClassifier or WANNRegressor object with the best genome (neural network)
        """
...
class WANNClassifier(BaseNEAT, ClassifierMixin):
    """WANN classifier
    """

    def __init__(self,
                 single_shared_weights=None,
    ...
    def _fitness_function(self, genomes, config):
        ...
        for genome_id, genome in genomes:
            reward = np.empty(len(self.single_shared_weights))
            for iVal in range(len(self.single_shared_weights)):
                # Set single-shared weight values
                for connection in genome.connections.values():
                    connection.weight = self.single_shared_weights[ iVal ]
                ...
                reward[ iVal ] = accuracy_score(self.y_, predictions)

            genome.fitness = np.mean(reward)
            genome.fitMax = np.max(reward)
    ...
    def _validate_input(self, X, y):
        """
        Encode target labels with value between 0 and n_classes-1 with LabelEncoder.
        Get unique class labels.
        Input validation for sklearn estimators.
...
class WANNRegressor(BaseNEAT, RegressorMixin):
    """WANN regressor
    """
    ...
    def _fitness_function(self, genomes, config):
        ...
        for genome_id, genome in genomes:
            ...
            for iVal in range(len(self.single_shared_weights)):
                ...
                reward[ iVal ] = r2_score(self.y_, predictions)
